...
test('counting projection derives monotone bounded progress and exact accessible seconds', () => {
  ...
  assert.equal(start.status, 'counting'); assert.equal(start.progress, 0); assert.equal(start.remainingSeconds, 9);
  assert.equal(start.visibleText, 'World cycle continues automatically');
  assert.equal(start.accessibleText, 'Next World starts automatically in 9 seconds. Any interaction cancels it.');
  assert.equal(start.active, true); assert.equal(generation, 1);
  assert.equal(advanceContinuation(state, 3250), false); const quarter = continuationPresentation(state);
  assert.equal(quarter.progress, .25); assert.equal(quarter.remainingSeconds, 7);
  assert.ok(quarter.progress >= start.progress && quarter.progress <= 1);
  assert.equal(advanceContinuation(state, 10_000), true); const firing = continuationPresentation(state);
...
test('hidden, cancelled, and disabled states preserve truthful non-color text', () => {
  const state = createContinuation(9000); startContinuation(state, 0);
  ...
  assert.match(paused.visibleText, /paused while hidden/); assert.match(paused.accessibleText, /7 seconds remaining/);
...
test('presentation cadence bounds styles near 30 Hz and text near one-second boundaries', () => {
  const authority = createContinuation(9000); startContinuation(authority, 0);
  ...
  assert.equal(visible, 1); assert.equal(accessible, 2, 'initial 9 seconds and the 8-second boundary only');
