...
test('planetary sky extends existing draws and has one bounded presentation authority', () => {
  ...
  assert.match(globe, /samplerCube uCloudField/); assert.match(canvasSky, /sampleAdvectedCloudField/);
  assert.match(globe, /rotateCloud\(normalize\(vPos\)/); assert.doesNotMatch(globe, /atan\(|uCloudPhase/);
  assert.match(canvasSky, /topo\.positions/); assert.doesNotMatch(canvasSky, /Math\.atan2|Math\.acos/);
  assert.match(canvasSky, /CANVAS_CLOUD_ANGLE_BUCKETS = 2048/); assert.match(fallback, /cloudSampleEpoch/);
  ...
  assert.match(policy, /CLOUD_SECONDARY_PERIOD_MS = 6_540_000/);
