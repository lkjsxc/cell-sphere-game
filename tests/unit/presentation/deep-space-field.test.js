/** Generated deep-space and star strata stay deterministic, broad, and bounded. */
...
test('the 256x128 RGB deep-space field is deterministic, chromatic, and broad', () => {
  ...
  assert.ok(a.maximumLuminance - a.minimumLuminance >= 28, `${a.minimumLuminance}/${a.maximumLuminance}`);
  assert.ok(a.meanLuminance >= 10 && a.meanLuminance <= 22, String(a.meanLuminance));
  ...
  assert.ok(chromatic > 8_000, String(chromatic));
  assert.ok(adjacent < distant * .18, `field resolved into pixel noise: ${adjacent}/${distant}`);
  assert.ok(Math.abs(deepSpaceInfluence(a, .2, .7) - deepSpaceInfluence(a, .8, .2)) > .25);
...
test('three star strata have ordered fixed budgets and varied material', () => {
  ...
  assert.deepEqual(STAR_BUDGETS, { eco: 124, balanced: 210, high: 300 });
  assert.deepEqual(STAR_STRATA_COUNTS.balanced, [160, 42, 8]);
  ...
  for (let star = 0; star < MAX_SKY_STARS; star++) {
    ...
    assert.ok(x >= .01 && x <= .99 && y >= .01 && y <= .99); assert.ok(size >= .4 && size <= 3.2);
    assert.ok(intensity >= .2 && intensity <= 1); warm += a[at + 4] < -.28; cool += a[at + 4] > .28;
    occupied.add(`${Math.floor(x * 12)}:${Math.floor(y * 7)}`);
  }
  assert.ok(warm >= 70 && cool >= 70, `${warm}/${cool}`); assert.ok(occupied.size >= 70, String(occupied.size));
