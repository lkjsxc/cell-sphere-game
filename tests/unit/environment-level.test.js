...
test('Environment Level schedule starts every world at zero and crosses exact thresholds', () => {
  ...
  assert.equal(environmentLevelAtTick('0'), '0');
  assert.equal(environmentLevelAtTick('1199'), '0');
  assert.equal(environmentTickForLevel('0'), '0');
  assert.equal(environmentTickForLevel('1'), ENVIRONMENT_LEVEL_OPENING_TICKS);
  assert.equal(environmentTickForLevel('2'), '1800');
  assert.equal(environmentTickForLevel('3'), '2400');
  assert.equal(environmentLevelAtTick('1200'), '1');
  assert.equal(environmentLevelAtTick('1799'), '1');
  assert.equal(environmentLevelAtTick('1800'), '2');
  const initial = environmentScheduleAtTick('0');
  assert.equal(initial.currentEnvironmentLevel, '0');
  assert.equal(initial.environmentLevelStartTick, '0');
  assert.equal(initial.nextEnvironmentLevelTick, ENVIRONMENT_LEVEL_OPENING_TICKS);
  assert.equal(initial.environmentLevelProgressQ, 0);
});
...
test('schedule is monotone and direct threshold inversion holds without a level table', () => {
  ...
  for (const level of levels) {
    ...
    assert.equal(environmentLevelAtTick(tick), level);
    if (priorTick !== null) assert.ok(BigInt(tick) > BigInt(priorTick), `${level} threshold did not advance`);
    priorTick = tick;
  }
  let previous = '0';
  for (const tick of ['0', '1', '1199', '1200', '1201', '1799', '1800', '999999']) {
    const level = environmentLevelAtTick(tick);
    assert.ok(BigInt(level) >= BigInt(previous), `${tick} regressed`);
    previous = level;
  }
});

test('schedule progress is bounded, deterministic, and resets at each transition', () => {
  const before = environmentProgressAtTick('1199');
  assert.equal(before.currentEnvironmentLevel, '0');
  assert.equal(before.ticksIntoLevel, '1199');
  assert.ok(before.environmentLevelProgressQ > 990_000);
  const atOne = environmentProgressAtTick('1200');
  assert.equal(atOne.currentEnvironmentLevel, '1');
  assert.equal(atOne.ticksIntoLevel, '0');
  assert.equal(atOne.environmentLevelProgressQ, 0);
  const middle = environmentProgressAtTick('1500');
  assert.equal(middle.currentEnvironmentLevel, '1');
  assert.equal(middle.environmentLevelProgressQ, 500_000);
  assert.equal(middle.ticksUntilNextLevel, '300');
});

test('schedule validation derives state and rejects forged redundant fields', () => {
  const state = environmentScheduleAtTick('2401');
  const valid = validateEnvironmentScheduleState(state);
  assert.equal(valid.valid, true);
  assert.equal(valid.environmentScheduleVersion, ENVIRONMENT_SCHEDULE_VERSION);
  ...
  assert.equal(forged.currentEnvironmentLevel, '3');
  assert.equal(forged.reason, 'derived-schedule-state');
});

test('huge canonical schedule values remain exact and expose only finite progress', () => {
  const level = `9${'0'.repeat(ENVIRONMENT_LEVEL_DOCUMENT_DIGIT_LIMIT - 2)}`;
  const tick = environmentTickForLevel(level);
  const state = environmentScheduleAtTick(tick);
  assert.equal(state.currentEnvironmentLevel, level);
  assert.equal(state.environmentLevelProgressQ, 0);
  assert.ok(state.nextEnvironmentLevelTick.length >= tick.length);
  assert.equal(environmentLevelAtTick(state.nextEnvironmentLevelTick), (BigInt(level) + 1n).toString());
});

test('legacy static-frontier evidence remains explicitly read-only migration input', () => {
  assert.deepEqual(['0', '1', '2', '3', '4', '5', '10', '11', '9007199254740993'].map(legacyEnvironmentFrontierForRuns),
    ['0', '0', '1', '2', '2', '2', '3', '4', '4']);
  assert.equal(environmentScheduleAtTick('0').currentEnvironmentLevel, '0');
});

test('direct pressure compiler remains finite over schedule-produced huge levels', () => {
...
test('rating projection is bounded and never requires Number conversion of a huge decimal', () => {
  assert.equal(pressureForNetRating('0'), 0);
  assert.equal(pressureForNetRating('1000'), 0.35);
