...
class GameApp {
  ...
  makeRenderer(seed, mode = 'world') {
    ...
    document.getElementById('trophy-next-button')?.addEventListener('click', () => this.startRun()); document.getElementById('trophy-evolution-button')?.addEventListener('click', () => this.enterMemory());
  ...
  bindCanvas() { const interrupt = () => { interruptCameraPolicy(this.cameraPolicy, performance.now()); if (this.state === 'result') this.pauseContinuation('gesture', true); }; this.input = bindGlobeInput(this.canvas, this.camera, { canInteract: () => ['title', 'running', 'result', 'memory', 'trophies'].includes(this.state),
