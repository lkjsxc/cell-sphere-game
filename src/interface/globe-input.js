...
export function bindGlobeInput(canvas, camera, options) {
  ...
  const down = (event) => {
    ...
    interrupt(); pointers.set(event.pointerId, { x: event.clientX, y: event.clientY,
      startX: event.clientX, startY: event.clientY, at: performance.now() });
  ...
  const finish = (event) => {
    ...
    if (!pointers.size) { pinchDistance = 0; options.onInteractionEnd?.(); }
    if (!pointer || invalidTap || !options.canInteract()) return;
    const moved = Math.hypot(event.clientX - pointer.startX, event.clientY - pointer.startY);
    if (moved <= 12 && performance.now() - pointer.at <= 520) options.onTap(event.clientX, event.clientY);
  };
