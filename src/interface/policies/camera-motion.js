...
export function endCameraDrag(state, now, kind = 'drag', observedNow = now) {
  ...
  if (!Number.isFinite(velocityX) || !Number.isFinite(velocityY) || !Number.isFinite(measuredSpeed)
    || measuredSpeed <= state.config.releaseThreshold) {
    stopAutomaticMotion(state); state.measuredReleaseSpeed = Number.isFinite(measuredSpeed) ? measuredSpeed : 0;
    state.mode = restingMode(state); return false;
  }
  state.velocityX = velocityX; state.velocityY = velocityY; state.measuredReleaseSpeed = measuredSpeed;
...
export function cameraMotionSnapshot(state) {
  return Object.freeze({ mode: state.mode, scene: state.scene, reduced: state.reduced, hidden: state.hidden,
    ...
    measuredReleaseSpeed: state.measuredReleaseSpeed,
