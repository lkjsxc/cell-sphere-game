...
import { cancelContinuation } from './continuation.js'; import * as ui from '../surfaces.js';
...
export function progressionTap(app, node) {
  if (app.state === 'memory') { const index = MEMORY_ATLAS_REVERSE[node]; if (index >= 0) selectSkill(app, MEMORY_NODES[index].id); return true; }
  if (app.state === 'trophies') { const index = TROPHY_ATLAS_REVERSE[node]; if (index >= 0) selectTrophy(app, TROPHIES[index].id); return true; }
  return false;
}
export function enterEvolution(app) { app.closeActiveOverlay(); app.adaptationEffects.clear(); cancelContinuation(app.continuation); app.el.countdown.textContent = '';
  app.flow.send('memory'); app.selectedNode = null; app.makeRenderer(0, 'memory'); app.memorySnapshot = buildMemorySnapshot(app.topo3, app.meta);
  if (app.memorySnapshot.focus) focusCamera(app.camera, app.memorySnapshot.focus); app.memoryUi.syncTree(app.meta);
  ui.showMemory(app.el, app.meta, availableSkills(app)); app.resize(false); }
...
export function enterTrophies(app) { app.closeActiveOverlay(); app.adaptationEffects.clear(); cancelContinuation(app.continuation); app.el.countdown.textContent = '';
  ...
  app.flow.send('trophies'); app.selectedNode = null; app.makeRenderer(0, 'trophies'); app.trophySnapshot = buildTrophySnapshot(app.topo2, app.meta, null, app.pendingTrophyIds);
  if (app.trophySnapshot.focus) focusCamera(app.camera, app.trophySnapshot.focus); app.trophyUi.sync(app.meta); ui.showTrophies(app.el, app.meta); app.resize(false);
