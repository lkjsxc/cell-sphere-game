/** Settings dialog binding. The controller owns persistence and pause leases. */
import { validateSettings } from '../platform/settings.js';

const byId = (id) => document.getElementById(id);

export function createSettingsSurface(options) {
  const dialog = /** @type {HTMLDialogElement} */ (byId('settings-dialog'));
  const form = /** @type {HTMLFormElement} */ (byId('settings-form'));
  const close = /** @type {HTMLButtonElement} */ (byId('settings-close'));
  const note = byId('settings-time-note');
  ...
  form.addEventListener('change', () => {
    const next = readForm(form, settings);
    settings = validateSettings(next); options.onChange(settings); render(form, settings);
  });
  ...
  byId('import-data')?.addEventListener('change', async (event) => {
    ...
    try { options.onAction('import', await file.text()); }
    catch { options.onAction('import-error'); }
  });
  ...
  return {
    ...
    open(worldContinues) {
      ...
      if (note) note.hidden = !worldContinues;
