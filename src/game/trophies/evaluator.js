...
import { validateTrophyFacts } from './facts.js';
...
export function reconcileTrophies(meta, archive, newFacts = null) {
  const aggregate = baseAggregate(meta); let evaluatedWorlds = 0;
  if (newFacts) { const facts = validateTrophyFacts(newFacts); if (facts) { mergeFacts(aggregate, facts); evaluatedWorlds = 1; } }
...
export function mergeFacts(a, f) {
  a.geographyMask |= f.geographyMask; a.lakeTypeMask |= f.lakeTypeMask; a.lakeSalinityMask |= f.lakeSalinityMask;
  const maxima = { survivalSeconds: f.survivalSeconds, peakCoverageBp: f.peakCoverageBp, sustainedCoverageBp: f.sustainedCoverageBp,
    ...
    breakAndMend: f.morph[1] && f.morph[2] ? 1 : 0, environmentExposureWorld: f.flags & 1 ? 1 : 0,
    coherentMajority: f.flags & 4 ? 1 : 0, unbrokenMajority: f.flags & 8 ? 1 : 0,
    ...
    lakeRegionPeak: f.lake[6], lakeLoopSeconds: f.lake[7], loopSurplusPeak: f.lake[8],
    ...
    scoreReachWorld: f.masteryFlags & 1 ? 1 : 0, scoreBalancedWorld: f.masteryFlags & 2 ? 1 : 0,
    scoreEnvironmentWorld: f.masteryFlags & 4 ? 1 : 0, scoreHabitatWorld: f.masteryFlags & 8 ? 1 : 0,
    scoreSixAxisWorld: f.masteryFlags & 16 ? 1 : 0, resourceRemainingBp: f.resourceRemainingBp,
    resourceRecoveredCells: f.resourceRecoveredCells, freshwaterSupportedSeconds: f.freshwaterSupportedSeconds,
    transformedCells: f.transformedCells, electrifiedCells: f.electrifiedCells, glacialLakeCells: f.glacialLakeCells,
    maritimeForestCells: f.maritimeForestCells, reach100: f.reach100, environmentPeakLevel: f.environmentPeakLevel,
    environmentTimeAtPeakTicks: f.environmentTimeAtPeakTicks, environmentPressureTicksQ: f.environmentPressureTicksQ };
  ...
  a.worldsWithLake += f.lake[2] >= 2 ? 1 : 0; a.totalReachGains += f.reach[0]; a.totalRegrowth += f.reach[2];
  a.totalLakeLivingSeconds += f.lake[4]; a.balancedWorlds += meets(q, [8500, 4200, 3000, 8500, 5000, 6500]) ? 1 : 0;
  a.autonomousWorlds += f.autonomous; a.scarcityWorlds += f.scarcityCause; a.resourceDepletedCells += f.resourceDepletedCells;
}

function serializeProgress(a) {
  const aggregate = {};
  for (const key of [...TROPHY_MAX_KEYS, ...TROPHY_SUM_KEYS]) aggregate[key] = finite(a[key]);
  return { version: 5, geographyMask: a.geographyMask, geographyVersion: 3,
