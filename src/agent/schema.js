...
export const AGENT_SAVE_SCHEMA = 1;
...
function canonical(value) {
  ...
  return Object.freeze({ schema: AGENT_SAVE_SCHEMA,
    ...
    worldOrdinal: meta.runs + 1, goal: GOALS.has(value?.goal) ? value.goal : 'balanced',
    meta, history, lastResult: validateLastResult(value?.lastResult) });
}

function validateLastResult(raw) {
  if (!raw || typeof raw !== 'object') return null;
  const worldOrdinal = integer(raw.worldOrdinal, 1); const score = integer(raw.score);
  if (!worldOrdinal || score === null) return null;
  const habitats = {};
  for (const key of ['lake', 'tundra', 'snowIce', 'shallowOcean', 'deepOcean'])
    habitats[key] = integer(raw.habitats?.[key]) ?? 0;
  return Object.freeze({ worldOrdinal, seed: validSeed(raw.seed) ? raw.seed : 0,
    ...
    rank: text(raw.rank, 24, 'Seed'), echoes: integer(raw.echoes) ?? 0,
    worldPotential: integer(raw.worldPotential) ?? 0, peakReach: fraction(raw.peakReach),
    ...
    resources: Object.freeze({ initial: finite(raw.resources?.initial), final: finite(raw.resources?.final),
      depletedCells: integer(raw.resources?.depletedCells) ?? 0 }), habitats: Object.freeze(habitats),
