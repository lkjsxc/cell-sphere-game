...
out float vFlux;
void main() {
  vec3 surface = normalize(mix(aPosA, aPosB, aCorner.x));
  vec3 segment = normalize(aPosB - aPosA);
  vec3 tangent = normalize(cross(surface, segment));
  float lift = 1.008 + dot(normalize(uEye), surface) * 0.0001;
  vec3 world = normalize(surface + tangent * (aCorner.y - 0.5) * aParams.x) * lift;
  vEdge = aCorner.y;
  vAlong = aCorner.x;
  vStress = aParams.y;
  vFlux = aParams.z;
...
void main() {
  ...
  vec3 healthy = mix(vec3(0.86, 0.76, 0.38), vec3(0.78, 0.73, 0.59), uMemory);
  vec3 stressed = mix(vec3(0.92, 0.34, 0.24), vec3(0.56, 0.50, 0.46), uMemory);
