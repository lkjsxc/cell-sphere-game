...
export async function runPlanetarySkyFixture(t) {
  ...
  ok(historyAndQuality.history.enabled && historyAndQuality.history.changed && historyAndQuality.history.stars && historyAndQuality.history.deepSpace
    && historyAndQuality.quality.eco === 124 && historyAndQuality.quality.high === 300
  ...
  return Object.freeze({ schema: 2, rendererPath: path.renderer, simulationPath: path.fallback ? 'fallback' : 'worker',
...
function visualExpression() {
  return `(async()=>{const a=window.__CELL_SPHERE_APP__,renderer=a.renderer,canvas=a.canvas,snapshot=a.snapshot,identity=a.worldIdentity;
    ...
    const checks={deepSpaceVisible:broad.changed>10000&&broad.mean>1,deepSpaceStable:fieldRepeat.maximum===0,
      starsVisible:star.changed>60&&star.mean>0,starsStable:repeat.maximum===0,starsNonLattice:starSpectrum.valid,
