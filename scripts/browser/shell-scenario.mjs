...
export async function runScenario(t) {
  ...
  ok(result.score > 0 && result.scene === 'world' && result.phase === 'result' && result.overlay === 'result' && result.surface === 'result'
    ...
    &&result.nextLabel==='Next World'&&result.countdown.startsWith('Next world in')&&result.visualSaves.length===1&&result.visualSaves[0],`terminal world failed: ${JSON.stringify(result)}`);
