...
export async function runScenario(t) {
  ...
  const historyTracks = await evaluate(`(()=>{const panel=document.getElementById('history-dialog'),body=panel.querySelector('.surface-body'),p=panel.getBoundingClientRect(),b=body.getBoundingClientRect();return {bodyBounded:b.top>=p.top&&b.bottom<=p.bottom+1,bodyOverflow:getComputedStyle(body).overflowY,tracks:getComputedStyle(panel).gridTemplateRows.split(' ').length}})()`);
  ok(historyTracks.bodyBounded && historyTracks.bodyOverflow === 'auto' && historyTracks.tracks === 2, `History shell tracks failed: ${JSON.stringify(historyTracks)}`);
