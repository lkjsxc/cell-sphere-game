...
export async function runScenario(t) {
  ...
  const cycleStart=await evaluate(`(()=>{const a=window.__CELL_SPHERE_APP__,root=document.getElementById('result-continuation'),box=()=>{const r=root.getBoundingClientRect();return{left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height}};window.__CONTINUATION_LIVE_MUTATIONS__=0;window.__CONTINUATION_LIVE_OBSERVER__=new MutationObserver(()=>window.__CONTINUATION_LIVE_MUTATIONS__++);window.__CONTINUATION_LIVE_OBSERVER__.observe(document.getElementById('live-region'),{childList:true,characterData:true,subtree:true});return{visible:document.getElementById('result-continuation-visible').textContent,accessible:document.getElementById('result-continuation-accessible').textContent,progress:Number(root.style.getPropertyValue('--continuation-progress')),rect:box(),audit:{...a.worldResourceAudit().continuationPresentation}}})()`);await wait(1100);
  const cycleEnd=await evaluate(`(()=>{const a=window.__CELL_SPHERE_APP__,root=document.getElementById('result-continuation'),r=root.getBoundingClientRect();window.__CONTINUATION_LIVE_OBSERVER__.disconnect();return{visible:document.getElementById('result-continuation-visible').textContent,accessible:document.getElementById('result-continuation-accessible').textContent,progress:Number(root.style.getPropertyValue('--continuation-progress')),rect:{left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height},audit:{...a.worldResourceAudit().continuationPresentation},liveMutations:window.__CONTINUATION_LIVE_MUTATIONS__}})()`);
  ...
  await evaluate(`(()=>{Object.defineProperty(document,'hidden',{value:true,configurable:true});document.dispatchEvent(new Event('visibilitychange'));return true})()`);const pausedStart=await evaluate(`(()=>{const a=window.__CELL_SPHERE_APP__,root=document.getElementById('result-continuation');return{state:a.continuation.status,progress:Number(root.style.getPropertyValue('--continuation-progress')),accessible:document.getElementById('result-continuation-accessible').textContent}})()`);await wait(500);const pausedEnd=await evaluate(`(()=>{const a=window.__CELL_SPHERE_APP__,root=document.getElementById('result-continuation');return{state:a.continuation.status,progress:Number(root.style.getPropertyValue('--continuation-progress'))}})()`);
  ...
  await evaluate(`(()=>{Object.defineProperty(document,'hidden',{value:false,configurable:true});document.dispatchEvent(new Event('visibilitychange'));return true})()`);await wait(80);const resumedCycle=await evaluate(`(()=>{const a=window.__CELL_SPHERE_APP__;return{state:a.continuation.status,progress:Number(document.getElementById('result-continuation').style.getPropertyValue('--continuation-progress')),announcement:document.getElementById('live-region').textContent}})()`);
  ...
  await trustedId(t, 'environment-level-button'); await trustedId(t, 'result-control'); await trustedId(t, 'menu-button'); const resultMenu = await evaluate(`(()=>({history:document.getElementById('menu-history').hidden,newWorld:document.getElementById('menu-new-world').hidden,auto:document.querySelector('#settings-form input[name="autoContinue"]').checked}))()`); ok(resultMenu.history && resultMenu.newWorld && resultMenu.auto, `terminal Menu exposed duplicate routes: ${JSON.stringify(resultMenu)}`);
  await trustedSelector(t,'#settings-form input[name="autoContinue"]');await wait(80);const disabledCycle=await evaluate(`(()=>{const a=window.__CELL_SPHERE_APP__;return{status:a.continuation.status,visible:document.getElementById('result-continuation-visible').textContent,accessible:document.getElementById('result-continuation-accessible').textContent,marker:getComputedStyle(document.querySelector('.continuation-marker')).display}})()`);
  ...
  await evaluate(`(()=>{const a=window.__CELL_SPHERE_APP__;a.applySettings({...a.settings,motion:'reduced'});return true})()`);const reducedCycle=await evaluate(`(()=>({marker:getComputedStyle(document.querySelector('.continuation-marker')).display,offset:getComputedStyle(document.getElementById('result-continuation-trace')).strokeDashoffset,text:document.getElementById('result-continuation-visible').textContent}))()`);
  ...
  await setMedia([{name:'forced-colors',value:'active'}]);const forcedCycle=await evaluate(`(()=>({active:matchMedia('(forced-colors: active)').matches,track:getComputedStyle(document.querySelector('.continuation-track')).stroke,trace:getComputedStyle(document.querySelector('.continuation-trace')).stroke,text:document.getElementById('result-continuation-visible').textContent}))()`);await setMedia([]);
  ...
  for (const [width, height, fontSize] of [[320, 568, ''], [360, 640, '32px'], [390, 844, ''], [1024, 600, '32px'], [1440, 900, '']]) {
    ...
    const layout = await evaluate(`(()=>{const container=document.querySelector('.hud-metrics'),controls=[...container.children].filter(x=>!x.hidden),rects=controls.map(x=>x.getBoundingClientRect()),tops=new Set(rects.map(r=>Math.round(r.top))),style=getComputedStyle(container),plain=rects.map(r=>({left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height})),actions=[...document.querySelectorAll('#result-dialog footer button')].map(x=>({label:x.textContent.trim(),...(()=>{const r=x.getBoundingClientRect();return{left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height}})()})),overlaps=list=>list.some((a,i)=>list.some((b,j)=>j>i&&!(a.right<=b.left||a.left>=b.right||a.bottom<=b.top||a.top>=b.bottom))),panel=document.getElementById('result-dialog'),body=panel.querySelector('.result-body'),footer=panel.querySelector('.result-actions'),cycle=document.getElementById('result-continuation'),trace=document.getElementById('result-continuation-trace'),box=e=>{const r=e.getBoundingClientRect();return{left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height}},panelBox=box(panel),footerBefore=box(footer),original=cycle.style.getPropertyValue('--continuation-progress'),originalOffset=trace.style.strokeDashoffset;cycle.style.setProperty('--continuation-progress','.1');trace.style.strokeDashoffset='.9';const cycleLow=box(cycle);cycle.style.setProperty('--continuation-progress','.9');trace.style.strokeDashoffset='.1';const cycleHigh=box(cycle);cycle.style.setProperty('--continuation-progress',original);trace.style.strokeDashoffset=originalOffset;body.scrollTop=body.scrollHeight;const footerAfter=box(footer),cycleBox=box(cycle);return {count:controls.length,minHeight:Math.min(...rects.map(r=>r.height)),bounded:rects.every(r=>r.left>=0&&r.right<=innerWidth&&r.top>=0&&r.bottom<=innerHeight),rows:tops.size,overlap:overlaps(rects),display:style.display,columns:style.gridTemplateColumns,rects:plain,actions,actionsBounded:actions.every(r=>r.left>=0&&r.right<=innerWidth&&r.top>=0&&r.bottom<=innerHeight),actionsOverlap:overlaps(actions),panel:panelBox,body:box(body),footerBefore,footerAfter,cycle:cycleBox,cycleLow,cycleHigh,visible:document.getElementById('result-continuation-visible').textContent,bodyScroll:body.scrollTop}})()`);
