...
| `browser-file-test.mjs` | `npm run test:browser:file` / `test:browser:canvas` / `test:browser:atmosphere*` / `test:browser:life-boundaries*` | Socket-free real Chrome harness for WebGL2, forced Canvas 2D, and controlled atmosphere/life-edge evidence. |
