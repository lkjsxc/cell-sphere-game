...
async function runCanvasScenario({ evaluate, screenshot, setViewport, poll, wait, errors }) {
  ...
  await evaluate(`(()=>{const a=window.__CELL_SPHERE_APP__;a.historySnapshot=null;a.historyPlaybackActive=false;delete a.__luminousDecaySnapshot;a.pause.set('browser-luminous',false);const speed=document.getElementById('speed-select');speed.value='256';speed.dispatchEvent(new Event('change'))})()`);
  ...
  if(![7,10].includes(selectedReady.status)||!selectedReady.action?.includes('Level'))throw new Error(`Canvas selected-ready state failed: ${JSON.stringify(selectedReady)}`);
  ...
  if(score<=0||atlas!==252||trophies.cells!==162||trophies.nodes!==96||errors.length)throw new Error('Canvas fallback state failed');
