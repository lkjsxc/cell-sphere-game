...
import { assertDockGeometry, assertSkillGeometry, captureTitleEvidence } from './browser-evidence.mjs'; export async function runScenario(t) {
  ...
  const started = performance.now(); let presentationPause = 0; let effectPauseStart = 0; ok(await poll(() => evaluate('window.__IN_APP__.state'), (value) => value === 'running', 5000), 'clock run did not start'); const dial0 = await evaluate(`(()=>[...document.querySelectorAll('.clock-hour,.clock-minute')].map(e=>getComputedStyle(e).transform))()`); await wait(220); const dial1 = await evaluate(`(()=>[...document.querySelectorAll('.clock-hour,.clock-minute')].map(e=>getComputedStyle(e).transform))()`); await evaluate("document.getElementById('pause-button').click()"); await wait(40); const frozen0 = await evaluate(`(()=>[...document.querySelectorAll('.clock-hour,.clock-minute')].map(e=>getComputedStyle(e).transform))()`); await wait(220); const frozen1 = await evaluate(`(()=>[...document.querySelectorAll('.clock-hour,.clock-minute')].map(e=>getComputedStyle(e).transform))()`); await evaluate("document.getElementById('pause-button').click()"); ok(dial0.every((value,index)=>value!==dial1[index]) && frozen0.every((value,index)=>value===frozen1[index]), 'dual clock hands did not move and freeze');
