...
const rows=Object.fromEntries(Object.keys(configs).map((key)=>[key,[]]));let uniqueOccupancyViolations=0;for(const [label,memory] of Object.entries(configs))for(let index=0;index<count;index++){
 const rc=new RunController({seed:0x510000+index,worldOrdinal:8,worldPotential:memory.worldPotential,memoryEffects:memory.effects,memoryConditionals:memory.conditionals,
  memoryUnlocks:memory.unlocks,habitatCapabilities:memory.habitatCapabilities});rc.start();rc.advance(4000);const r=rc.buildResult(),o=r.habitatOccupancy,total=o.reduce((a,b)=>a+b,0);
...
report.valid=!uniqueOccupancyViolations&&Object.values(unlockIds).every(Boolean)&&gated.every((key)=>report.configurations.fresh[key].max===0)&&report.configurations.fresh.blocked.median>0
 &&gated.every((key)=>report.configurations.full[key].median>0)&&report.configurations.full.marineShare.median<.6&&report.configurations.full.peak.median<.85
