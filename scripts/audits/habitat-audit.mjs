...
import { EVOLUTION_ARCHETYPES, compileEvolution, evolutionRunConfiguration } from '../../src/game/skills/index.js'; import { BIOME } from '../../src/world/constants.js';
import { evolutionLevelsForCells, evolutionPathToArchetype, evolutionRepresentativeLevels } from '../lib.mjs';
...
const compileCells = (cells) => compileEvolution({ evolutionLevels: evolutionLevelsForCells(cells) });
const fresh = compileCells([]); const breadth = compileEvolution({ evolutionLevels: evolutionRepresentativeLevels('1') });
const keys = ['LAKE_ACCESS', 'TUNDRA_ACCESS', 'SNOW_ICE_ACCESS', 'SHALLOW_OCEAN_EDGE_ACCESS', 'SHALLOW_OCEAN_ACCESS', 'DEEP_OCEAN_ACCESS'];
const unlockIds = Object.fromEntries(keys.map((key) => [key, EVOLUTION_ARCHETYPES.find((archetype) => archetype.effects.some((effect) => effect.kind === 'habitat' && effect.capability === key))?.id]));
const lineage = (...ids) => [...new Set(ids.flatMap(evolutionPathToArchetype))];
...
const configs = { fresh, breadth, ...Object.fromEntries(Object.entries(packages).map(([key, cells]) => [key, compileCells(cells)])) };
