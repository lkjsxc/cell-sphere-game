...
import { MEMORY_NODES, MEMORY_NODE_IDS, availableMemoryNodes, compileMemory, memoryPurchasePreview,
...
let meta = { ...defaultMeta(), echoBalance: 1_000_000 }; let spent = 0; const impacts = []; const impactById = new Map(); const potentialGains = [];
while (meta.memoryNodes.length < MEMORY_NODES.length) {
  ...
  impacts.push(impact); impactById.set(node.id, impact); potentialGains.push(preview.potentialGain);
...
const potential = potentialGains.slice().sort((a,b)=>a-b); const degreeCounts = Object.fromEntries([...new Set(topo.degree)].map((d)=>[d,[...topo.degree].filter((v)=>v===d).length]));
...
const report = { elapsedMs:Number((performance.now()-started).toFixed(1)), topology:{frequency:5,cells:topo.nodeCount,edges:topo.edgeCount,faces:topo.triCount,
  ...
  potential:{fresh:16000,full:unlocked.worldPotential,gain:{min:potential[0],median:q(potential,.5),p95:q(potential,.95),max:potential.at(-1)}},
