...
- Starting user work: the companion `AGENTS.md` modification and untracked `docs/campaigns/202608282058.md`; both were inspected, are nonconflicting, and remain preserved. The companion contract will be committed with the implementation; the original user artifact remains untouched.
...
- Evidence still required: matched controlled life-edge fixture and repeat-noise calibration, fallback/context-loss final paths, final same-host timing, broad verification, CI, Pages, deployed bytes/browser, and physical-device classification.
- Completed coherent phases: Milestone 0 repository/external reconciliation, ownership trace, focused authority baseline, and trusted WebGL2/Canvas browser baseline; Milestone 1 two-byte shared edge projection and exhaustive semantic oracle. The old production rendering remains authoritative at this boundary.
- Milestone 1 focused evidence: 5/5 `life-edges` tests pass alongside 19/19 affected renderer tests. All 36 categorical endpoint pairs and reversals prove symmetry, precedence, relation, finite bounded intensity, deterministic repetition, canonical `edgeA`/`edgeB` ordering, four-vertex expansion, and absence of time/camera/RNG/simulation dependencies.
- Selected packing: one byte packs dominant state plus relation; one byte carries safely normalized intensity. The canonical 7,680-edge projection is 15,360 bytes. The existing four boundary-quad vertices will receive a 61,440-byte expanded `UNSIGNED_BYTE vec2` GPU attribute without a new draw.
- Exact next coherent step: atomically wire both backends to the shared projection, delete ordinary living/frontier whole-cell authority, and add the controlled production-browser fixture.
