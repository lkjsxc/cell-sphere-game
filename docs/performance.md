...
fallback/Canvas `5.3/0.7/2.1/1.7 ms`. Compared with the starting measurements
`18.0/0.7/2.0/1.6`, `21.7/0.6/1.8/1.6`, and `5.3/0.4/1.8/1.8 ms`, no stable
material render regression remains; the small Canvas projection/update deltas
are sub-millisecond. One earlier fallback/WebGL2 sample measured
`21.4/0.7/1.9/3.7 ms`; its steady outlier did not reproduce in later exact runs
(`1.6` and `1.1 ms`) or the pre-commit run (`1.1 ms`). Layout/geometry identities
remain stable and unchanged frames rebuild zero edge bytes.
